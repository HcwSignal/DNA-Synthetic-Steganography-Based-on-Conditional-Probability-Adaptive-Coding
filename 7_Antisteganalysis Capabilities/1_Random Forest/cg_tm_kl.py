...
def find_bpn(path):
    ...
    for line in lines:
        line_ = line.split(' ')
        for elements in line_:
            if elements.find('rl:') == 0:
                Bpn = float(elements[3:])
                BPN.append(Bpn)

    bpn_mean = np.mean(np.array(BPN))

    if float(bpn_mean) != float(bpn):
        print('path:{}, bpn extracting error'.format(str(path)))
        print('bpn_mean:',bpn_mean)
        print('bpn:', bpn)
        bpn = bpn_mean
